...
class LogisticRegression:
    ...
    def fit(self,x_train,y_train,eta = 0.01,n_iters = 1e4):
        ...
        :return:
        """
        assert  x_train.shape[0] == y_train.shape[0],'两者大小相同'
        def cost(theta,x_b,y):
            ...
            :return:L(w)的值
            """
            y_hat = self.sigmond(np.array(x_b.dot(theta),dtype=np.float32));
            try:
        ...
        def dcost(theta,x_b,y):
            ...
            :return:
            """
            return x_b.T.dot(self.sigmond(np.array(x_b.dot(theta), dtype=np.float32)) - y) / len(x_b)
    ...
    def predict_proba(self, x_predict):
        ...
        return self.sigmond(np.array(x_b.dot(self._theta), dtype=np.float32))
